/*
Copyright (C) 2019-2026 TARAKHOMYN YURIY IVANOVYCH
All rights reserved.

Licensed under the Apache License, Version 2.0 (the "License");
you may not use this file except in compliance with the License.
You may obtain a copy of the License at

    http://www.apache.org/licenses/LICENSE-2.0

Unless required by applicable law or agreed to in writing, software
distributed under the License is distributed on an "AS IS" BASIS,
WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
See the License for the specific language governing permissions and
limitations under the License.
*/

/*
Автор:    Тарахомин Юрій Іванович
Адреса:   Україна, м. Львів
Сайт:     accounting.org.ua
*/

/*

Спільна форма - Рух документу по регістрах

*/

using Gtk;
using AccountingSoftware;
using GeneratedCode;
using StorageAndTrade.РегістриНакопичення;

namespace StorageAndTrade;

/// <summary>
/// Рух документу по регістрах
/// </summary>
[GObject.Subclass<InterfaceGtk4.CommonForms_DocumentMovementThroughRegisters>]
partial class CommonForms_DocumentMovementThroughRegisters : InterfaceGtk4.CommonForms_DocumentMovementThroughRegisters
{
    public static CommonForms_DocumentMovementThroughRegisters New()
    {
        CommonForms_DocumentMovementThroughRegisters view = NewWithProperties([]);
        view.NotebookFunc = Program.BasicForm?.NotebookFunc;

        return view;
    }

    public static async void Create(DocumentPointer documentPointer)
    {
        CommonForms_DocumentMovementThroughRegisters page = New();
        page.NotebookFunc?.CreatePage("Проводки", () => page);
        await page.Fill(documentPointer);
    }

    /// <summary>
    /// Контрол який відображає документ для якого виводяться рухи по регістрах
    /// </summary>
    /// <param name="documentPointer">Документ вказівник</param>
    /// <returns>Widget</returns>
    protected override Widget Document_PointerControl(DocumentPointer documentPointer)
    {
        CompositePointerControl control = CompositePointerControl.New();
        control.Caption = "Документ:";
        control.Pointer = documentPointer.GetBasis();
        control.ClearSensetive = false;
        control.TypeSelectSensetive = false;
        control.WidthPresentation = 500;

        return control;
    }

    /// <summary>
    /// Заповнення
    /// </summary>
    /// <param name="documentPointer">Відбір по документу</param>
    /// <returns></returns>
    public async Task Fill(DocumentPointer documentPointer)
    {
        NotebookFunc?.SpinnerOn(GetName());

        //Додавання контрола документу на форму
        AddDocument(documentPointer);

        //Доступні регістри для даного документу
        foreach (string regAccumName in Config.Kernel.Conf.Documents[documentPointer.TypeDocument].AllowRegisterAccumulation)
        {
            switch (regAccumName)
            {
                case "ТовариНаСкладах":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ТовариНаСкладах_СписокМіні page = ТовариНаСкладах_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Товари на складах", getForm);
                        break;
                    }
                case "ПартіїТоварів":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ПартіїТоварів_СписокМіні page = ПартіїТоварів_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Партії товарів", getForm);
                        break;
                    }
                case "ЗамовленняКлієнтів":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ЗамовленняКлієнтів_СписокМіні page = ЗамовленняКлієнтів_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Замовлення клієнтів", getForm);
                        break;
                    }
                case "РозрахункиЗКлієнтами":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            РозрахункиЗКлієнтами_СписокМіні page = РозрахункиЗКлієнтами_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Розрахунки з клієнтами", getForm);
                        break;
                    }
                case "ВільніЗалишки":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ВільніЗалишки_СписокМіні page = ВільніЗалишки_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Вільні залишки", getForm);
                        break;
                    }
                case "ЗамовленняПостачальникам":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ЗамовленняПостачальникам_СписокМіні page = ЗамовленняПостачальникам_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Замовлення постачальникам", getForm);
                        break;
                    }
                case "РозрахункиЗПостачальниками":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            РозрахункиЗПостачальниками_СписокМіні page = РозрахункиЗПостачальниками_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Розрахунки з постачальниками", getForm);
                        break;
                    }
                case "РухКоштів":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            РухКоштів_СписокМіні page = РухКоштів_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Рух коштів", getForm);
                        break;
                    }
                case "РухКоштівККМ":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            РухКоштівККМ_СписокМіні page = РухКоштівККМ_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Рух коштів ККМ", getForm);
                        break;
                    }
                case "Закупівлі":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            Закупівлі_СписокМіні page = Закупівлі_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Закупівлі", getForm);
                        break;
                    }
                case "Продажі":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            Продажі_СписокМіні page = Продажі_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Продажі", getForm);
                        break;
                    }
                case "ТовариВКомірках":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            ТовариВКомірках_СписокМіні page = ТовариВКомірках_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();

                            return page;
                        }

                        await AddForm("Товари в комірках", getForm);
                        break;
                    }
                case "БухгалтерськіОперації":
                    {
                        async Task<Widget> getForm(bool small = true)
                        {
                            БухгалтерськіОперації_СписокМіні page = БухгалтерськіОперації_СписокМіні.New();
                            page.WhereList = [new("owner", Comparison.EQ, documentPointer.UniqueID.UGuid)];
                            if (small) page.SetHeight(300);
                            await page.SetValue();
                            return page;
                        }

                        await AddForm("Бухгалтерські операції", getForm);
                        break;
                    }
            }
        }

        NotebookFunc?.SpinnerOff(GetName());
    }
}