...
static class ФункціїДляДокументів
{
    ...
    public static UuidAndText ЗадатиТипАналітики(string Субконто) => new(Субконто);
    ...
    public static async Task<БухгалтерськаОперація_Pointer> СтворитиДокументБухгалтерськаОперація(БухгалтерськаОперація_Pointer БухОпераціяВказівник, ДляНовогоДокументуБухгалтерськаОперація Реквізити)
    {
        БухгалтерськаОперація_Object? БухОпераціяОбєкт;
        if (БухОпераціяВказівник.IsEmpty())
        {
            БухОпераціяОбєкт = new();
            await БухОпераціяОбєкт.New();
        }
        else
            БухОпераціяОбєкт = await БухОпераціяВказівник.GetDocumentObject();

        if (БухОпераціяОбєкт != null)
        {
            //Бух операція на секунду пізніше від базового документу
            БухОпераціяОбєкт.ДатаДок = Реквізити.ДатаДок.AddSeconds(1);
            БухОпераціяОбєкт.Організація = Реквізити.Організація;
            БухОпераціяОбєкт.Підстава = Реквізити.Підстава;
            await БухОпераціяОбєкт.Save();

            return БухОпераціяОбєкт.GetDocumentPointer();
        }
        else
            return new();
    }
    ...
    public record ДляНовогоДокументуБухгалтерськаОперація
    {
        ...
        public UuidAndText Підстава { get; init; } = new();
    }

    public static async Task ПровестиДокументБухгалтерськаОперація(БухгалтерськаОперація_Pointer БухОпераціяВказівник)
    {
        if (!БухОпераціяВказівник.IsEmpty())
        {
            БухгалтерськаОперація_Object? БухОпераціяОбєкт = await БухОпераціяВказівник.GetDocumentObject(true);
            if (БухОпераціяОбєкт != null)
            {
                if (!await БухОпераціяОбєкт.SpendTheDocument(БухОпераціяОбєкт.ДатаДок))
                    throw new Exception($"Не вдалось провести бухгалтерський документ {БухОпераціяОбєкт.Назва}");
            }
            else
                throw new Exception($"Не вдалось прочитати бухгалтерський документ {БухОпераціяВказівник}");
        }
    }

    public static async Task ВідмінитиПроведенняДокументуБухгалтерськаОперація(БухгалтерськаОперація_Pointer БухОпераціяВказівник)
    {
        if (!БухОпераціяВказівник.IsEmpty()) await БухОпераціяВказівник.ClearSpendTheDocument();
    }
    ...
    public static async Task<ПартіяТоварівКомпозит_Pointer> ОтриматиПартіюТоварівКомпозит(
        ...
        ВиготовленняПродукції_Object? ДокументВиготовленняПродукції)
    {
        ...
        switch (ТипДокументу)
        {
            ...
            case ТипДокументуПартіяТоварівКомпозит.ВведенняЗалишків when ДокументВведенняЗалишків != null:
                ...
                case ТипДокументуПартіяТоварівКомпозит.ВиготовленняПродукції when ДокументВиготовленняПродукції != null:
                {
                    партіяТоварівКомпозитНовий.Дата = ДокументВиготовленняПродукції.ДатаДок;
                    партіяТоварівКомпозитНовий.Назва = ДокументВиготовленняПродукції.Назва;
                    партіяТоварівКомпозитНовий.ВиготовленняПродукції = ДокументВиготовленняПродукції.GetDocumentPointer();
                    break;
                }
        }
