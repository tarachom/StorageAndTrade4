...
static class ФункціїДляДокументів
{
    ...
    {
        БухгалтерськаОперація_Object? БухОперація;
        ...
        if (БухОперація != null)
        {
            БухОперація.ДатаДок = Реквізити.ДатаДок.AddSeconds(1);
    ...
    public static async Task<ПартіяТоварівКомпозит_Pointer> ОтриматиПартіюТоварівКомпозит(
        ...
        ПоступленняТоварівТаПослуг_Object? ДокументПоступлення,
        ВведенняЗалишків_Object? ДокументВведенняЗалишків)
    {
        ...
        ПартіяТоварівКомпозит_Object партіяТоварівКомпозитНовий = new();
