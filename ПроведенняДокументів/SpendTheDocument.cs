...
class СпільніФункції
{
    ...
    public static async ValueTask<SelectRequest_Record> ОтриматиСписокНаявнихПартій(
        ...
        decimal Кількість)
    {
        Перелічення.МетодиСписання МетодСписання;

        if (Константи.ПартіїТоварів.МетодСписанняПартій_Const == Перелічення.МетодиСписання.FIFO)
            МетодСписання = Перелічення.МетодиСписання.FIFO;
        else if (Константи.ПартіїТоварів.МетодСписанняПартій_Const == Перелічення.МетодиСписання.LIFO)
            МетодСписання = Перелічення.МетодиСписання.LIFO;
        else
            МетодСписання = Перелічення.МетодиСписання.FIFO;

        string МетодСортування = МетодСписання == Перелічення.МетодиСписання.FIFO ? "ASC" : "DESC";
