...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
  ...
  <xsl:template name="ClearRegAccumFunction">
        ...
        async Task ClearRegAccum()
        {
          ...
          <xsl:otherwise>Task ClearRegAccum() =&gt; Task.CompletedTask;</xsl:otherwise>
        </xsl:choose>
  ...
  <xsl:template name="DirectoryOrDocumentSelectFields">
        <!-- Функція для отримання значення поля з колекції вибраних полів -->
        ...
        <xsl:for-each select="Fields/Field">
          ...
          <xsl:text> </xsl:text>
          <xsl:value-of select="Name"/> { get { var obj = getValue("<xsl:value-of select="NameInTable"/>"); return <xsl:call-template name="ReadFieldValue2">
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>
{
    ...
    {        
        ...
        */
        public static async Task&lt;CompositePointerPresentation_Record&gt; CompositePointerPresentation(UuidAndText uuidAndText)
        {
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.Константи
{
    ...
    {       
        ...
        <xsl:value-of select="Name"/>()
        {
            var recordResult = await Config.Kernel.DataBase.SelectConstants(SpecialTables.Constants, "<xsl:value-of select="NameInTable"/>");
        ...
        public class <xsl:value-of select="$TablePartFullName"/>_TablePart : ConstantsTablePart
        {
            ...
            public async Task Save(bool clear_all_before_save /*= true*/) 
            {
            ...
            public async Task Remove(Record record)
            {
                await base.BaseRemove(record.UID);
                Records.RemoveAll(item =&gt; record.UID == item.UID);
            }

            public async Task RemoveAll(List&lt;Record&gt; records)
            {
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.Довідники
{
    ...
    {
        public <xsl:value-of select="$DirectoryName"/>_Select() : base(Config.Kernel, "<xsl:value-of select="Table"/>", <xsl:value-of select="$DirectoryName"/>_Const.TYPE, <xsl:value-of select="$DirectoryName"/>_Const.PRESENTATION_FIELDS) { }        
    ...
    {
        public <xsl:value-of select="$DirectoryName"/>_SelectHierarchical() : base(Config.Kernel, "<xsl:value-of select="Table"/>", <xsl:value-of select="$DirectoryName"/>_Const.TYPE, "<xsl:value-of select="Fields/Field[Name = $ParentField]/NameInTable"/>", "<xsl:value-of select="$IsFolderField_NameInTable"/>", <xsl:value-of select="$DirectoryName"/>_Const.PRESENTATION_FIELDS) { }        
        public async Task&lt;bool&gt; Select() =&gt; await base.BaseSelect();
        ...
        public bool MoveNext() { if (base.MoveToPosition() &amp;&amp; base.CurrentPointerPositionHierarchical.HasValue) { 
          Current = new <xsl:value-of select="$DirectoryName"/>_Pointer(base.CurrentPointerPositionHierarchical.Value.UniqueID, base.CurrentPointerPositionHierarchical.Value.Fields, base.CurrentPointerPresentation); 
    ...
    public class <xsl:value-of select="$TablePartFullName"/>_TablePart : DirectoryTablePart
    {
        ...
        public async Task Read()
        {
            ...
            foreach (Dictionary&lt;string, object&gt; fieldValue in base.FieldValueList) 
            {
                ...
                Records.Add(record);
                <xsl:if test="count(Fields/Field[Type = 'pointer' or Type = 'composite_pointer']) != 0">
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.Документи
{
    ...
    {		
        public <xsl:value-of select="$DocumentName"/>_Select() : base(Config.Kernel, "<xsl:value-of select="Table"/>", <xsl:value-of select="$DocumentName"/>_Const.TYPE, <xsl:value-of select="$DocumentName"/>_Const.PRESENTATION_FIELDS) { }
        public async Task&lt;bool&gt; Select() =&gt; await base.BaseSelect();
        public async Task&lt;bool&gt; SelectSingle() { if (await base.BaseSelectSingle()) { MoveNext(); return true; } else { Current = null; return false; } }
    ...
    {
        public static async Task ToXmlFile(<xsl:value-of select="$DocumentName"/>_Pointer <xsl:value-of select="$DocumentName"/>, string pathToSave)
        {
        <xsl:choose>
          ...
          <xsl:otherwise>await Task.CompletedTask;</xsl:otherwise>
        </xsl:choose>
        }
    }
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.Журнали
{
    ...
    public class JournalSelect: AccountingSoftware.JournalSelect
    {
        ...
        public async Task&lt;DocumentObject?&gt; GetDocumentObject(bool readAllTablePart = true)
        {
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.РегістриВідомостей
{
    ...
    public class <xsl:value-of select="$RegisterName"/>_RecordsSet : RegisterInformationRecordsSet
    {
        ...
        public void FillJoin(string[]? orderFields = null)
        {
            ...
            <xsl:for-each select="(DimensionFields|ResourcesFields|PropertyFields)/Fields/Field">
                <xsl:choose>
                  <xsl:when test="Type = 'pointer'">
                      /* pointer */
                      <xsl:value-of select="substring-before(Pointer, '.')"/>.<xsl:value-of select="substring-after(Pointer, '.')"/>_Pointer.GetJoin(QuerySelect, <xsl:value-of select="$RegisterName"/>_Const.<xsl:value-of select="Name"/>, <xsl:value-of select="$RegisterName"/>_Const.TABLE, "join_tab_<xsl:value-of select="position()"/>", "<xsl:value-of select="Name"/>");
                  </xsl:when>
                  <xsl:when test="Type = 'composite_pointer'">
                      /* composite_pointer */
        ...
        public async Task Save(DateTime period, UuidAndText owner)
        {
            await base.BaseBeginTransaction();
    ...
    public class <xsl:value-of select="$RegisterName"/>_Object : RegisterInformationObject
    {
        ...
        public async Task&lt;bool&gt; Read(UniqueID uid)
        {
            if (await BaseRead(uid))
            {
                <xsl:for-each select="(DimensionFields|ResourcesFields|PropertyFields)/Fields/Field">
...
namespace <xsl:value-of select="Configuration/NameSpaceGeneratedCode"/>.РегістриНакопичення
{
    public static class VirtualTablesСalculation
    {
        ...
        public static async Task Execute(DateTime period, string regAccumName)
        {
            ...
            <xsl:if test="$QueryAllCountCalculation = 0">
                <xsl:text>await Task.CompletedTask;</xsl:text>
            </xsl:if>
        }

        /* Функція для обчислення підсумкових віртуальних таблиць */
        public static async Task ExecuteFinalCalculation(List&lt;string&gt; regAccumNameList)
        {
