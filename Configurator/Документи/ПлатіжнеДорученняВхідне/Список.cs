...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПлатіжнеДорученняВхідне_Записи;
using Функції = StorageAndTrade.ПлатіжнеДорученняВхідне_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalFull>("List_326eAVX4nXSRYL8dGRob1A")]
public partial class ПлатіжнеДорученняВхідне_Список : DocumentFormJournalFull
{
    partial void Initialize()
    {
        TypeName = ПлатіжнеДорученняВхідне_Const.POINTER;
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50, Pages.StartingPosition.End);
    }

    public static ПлатіжнеДорученняВхідне_Список New()
    {
        ПлатіжнеДорученняВхідне_Список list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПлатіжнеДорученняВхідне_Pointer(uid));
    }
