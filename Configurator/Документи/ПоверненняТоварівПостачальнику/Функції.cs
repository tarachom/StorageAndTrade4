...
static class ПоверненняТоварівПостачальнику_Функції
{
    ...
    {
        ПоверненняТоварівПостачальнику_Елемент page = new()
        {
    ...
    public static async ValueTask OpenPageList(UnigueID? unigueID = null, Action<UnigueID>? сallBack_OnSelectPointer = null)
    {
        ...
        Program.BasicForm?.NotebookFunc.CreatePage(ПоверненняТоварівПостачальнику_Const.FULLNAME, page);
        await page.SetValue();
    }

    public static async ValueTask SetDeletionLabel(UnigueID unigueID)
    {
        ПоверненняТоварівПостачальнику_Pointer Вказівник = new(unigueID);
