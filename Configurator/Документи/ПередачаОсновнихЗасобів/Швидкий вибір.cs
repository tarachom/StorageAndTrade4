...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПередачаОсновнихЗасобів_Записи;
using Функції = StorageAndTrade.ПередачаОсновнихЗасобів_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_2W6eAQkBFX6aKL6rtFmA")]
public partial class ПередачаОсновнихЗасобів_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПередачаОсновнихЗасобів_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПередачаОсновнихЗасобів_ШвидкийВибір New()
    {
        ПередачаОсновнихЗасобів_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПередачаОсновнихЗасобів_Pointer(uid));
    }
