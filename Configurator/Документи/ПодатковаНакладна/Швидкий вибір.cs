...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПодатковаНакладна_Записи;
using Функції = StorageAndTrade.ПодатковаНакладна_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_4m6eAXzAonmg7WjsVfWOaw")]
public partial class ПодатковаНакладна_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПодатковаНакладна_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПодатковаНакладна_ШвидкийВибір New()
    {
        ПодатковаНакладна_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПодатковаНакладна_Pointer(uid));
    }
