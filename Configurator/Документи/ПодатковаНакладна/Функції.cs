...
static class ПодатковаНакладна_Функції
{
    public static List<Where> Відбори(string searchText)
    {
        ...
        [
            ...
            //Назва
            new Where(ПодатковаНакладна_Const.Назва, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
                    
            //Коментар
            new Where(Comparison.OR, ПодатковаНакладна_Const.Коментар, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
    ...
    {
        ПодатковаНакладна_Елемент page = ПодатковаНакладна_Елемент.New();
    ...
    {
        ПодатковаНакладна_Список page = ПодатковаНакладна_Список.New();
        page.DocumentPointerItem = uniqueID;
        page.CallBack_OnSelectPointer = сallBack_OnSelectPointer;

        Program.BasicForm?.NotebookFunc.CreatePage(ПодатковаНакладна_Const.FULLNAME, page);
        await page.SetValue();
    }

    public static async Task SetDeletionLabel(UniqueID uniqueID)
    {
        ПодатковаНакладна_Pointer Вказівник = new(uniqueID);
