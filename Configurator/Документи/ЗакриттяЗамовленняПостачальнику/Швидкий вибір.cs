...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ЗакриттяЗамовленняПостачальнику_Записи;
using Функції = StorageAndTrade.ЗакриттяЗамовленняПостачальнику_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_98LSx63QDUWLqbMJMUf0cQ")]
public partial class ЗакриттяЗамовленняПостачальнику_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ЗакриттяЗамовленняПостачальнику_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ЗакриттяЗамовленняПостачальнику_ШвидкийВибір New()
    {
        ЗакриттяЗамовленняПостачальнику_ШвидкийВибір list = NewWithProperties([]);
    ...
    protected override void ReportSpendTheDocument(UniqueID[] uniqueID)
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ЗакриттяЗамовленняПостачальнику_Pointer(uid));
    }
