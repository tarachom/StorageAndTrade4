...
/*     
        Додаток1ДоПодатковоїНакладної.cs
        Список
*/
...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.Додаток1ДоПодатковоїНакладної_Записи;
using Функції = StorageAndTrade.Додаток1ДоПодатковоїНакладної_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_nNndd1qv5EKJ2EATATzFoQ")]
public partial class Додаток1ДоПодатковоїНакладної_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = Додаток1ДоПодатковоїНакладної_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static Додаток1ДоПодатковоїНакладної_ШвидкийВибір New()
    {
        Додаток1ДоПодатковоїНакладної_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new Додаток1ДоПодатковоїНакладної_Pointer(uid));
    }
