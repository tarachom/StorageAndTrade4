...
static class Додаток1ДоПодатковоїНакладної_Функції
{
    ...
    {
        Додаток1ДоПодатковоїНакладної_Object Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            Додаток1ДоПодатковоїНакладної_Object Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        Додаток1ДоПодатковоїНакладної_Object? Обєкт = await new Додаток1ДоПодатковоїНакладної_Pointer(uniqueID).GetDocumentObject(true);
