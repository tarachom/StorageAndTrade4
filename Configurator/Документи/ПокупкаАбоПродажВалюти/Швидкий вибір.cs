...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПокупкаАбоПродажВалюти_Записи;
using Функції = StorageAndTrade.ПокупкаАбоПродажВалюти_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_5G6eAZoga3qm9jJC7jMQxw")]
public partial class ПокупкаАбоПродажВалюти_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПокупкаАбоПродажВалюти_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПокупкаАбоПродажВалюти_ШвидкийВибір New()
    {
        ПокупкаАбоПродажВалюти_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПокупкаАбоПродажВалюти_Pointer(uid));
    }
