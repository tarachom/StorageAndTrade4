...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПрийняттяДоОбліку_Записи;
using Функції = StorageAndTrade.ПрийняттяДоОбліку_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_7W6eATcWRHKo3OxeOQN1QA")]
public partial class ПрийняттяДоОбліку_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПрийняттяДоОбліку_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПрийняттяДоОбліку_ШвидкийВибір New()
    {
        ПрийняттяДоОбліку_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПрийняттяДоОбліку_Pointer(uid));
    }
