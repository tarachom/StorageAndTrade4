...
/*     
        ПереміщенняМатеріалівВЕксплуатації.cs
        Список
*/
...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПереміщенняМатеріалівВЕксплуатації_Записи;
using Функції = StorageAndTrade.ПереміщенняМатеріалівВЕксплуатації_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_oW6eAU8van2h0zdIRGeftA")]
public partial class ПереміщенняМатеріалівВЕксплуатації_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПереміщенняМатеріалівВЕксплуатації_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПереміщенняМатеріалівВЕксплуатації_ШвидкийВибір New()
    {
        ПереміщенняМатеріалівВЕксплуатації_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПереміщенняМатеріалівВЕксплуатації_Pointer(uid));
    }
