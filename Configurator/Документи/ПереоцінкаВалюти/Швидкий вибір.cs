...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПереоцінкаВалюти_Записи;
using Функції = StorageAndTrade.ПереоцінкаВалюти_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_3W6eAYcxHHuEyUFc8jK0Q")]
public partial class ПереоцінкаВалюти_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПереоцінкаВалюти_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПереоцінкаВалюти_ШвидкийВибір New()
    {
        ПереоцінкаВалюти_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПереоцінкаВалюти_Pointer(uid));
    }
