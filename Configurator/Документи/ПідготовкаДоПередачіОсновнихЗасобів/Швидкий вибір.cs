...
/*     
        ПідготовкаДоПередачіОсновнихЗасобів.cs
        Список
*/
...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПідготовкаДоПередачіОсновнихЗасобів_Записи;
using Функції = StorageAndTrade.ПідготовкаДоПередачіОсновнихЗасобів_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_5W6eAaTHbHK5FnBHZhT6g")]
public partial class ПідготовкаДоПередачіОсновнихЗасобів_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПідготовкаДоПередачіОсновнихЗасобів_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПідготовкаДоПередачіОсновнихЗасобів_ШвидкийВибір New()
    {
        ПідготовкаДоПередачіОсновнихЗасобів_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПідготовкаДоПередачіОсновнихЗасобів_Pointer(uid));
    }
