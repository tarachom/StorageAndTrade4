...
static class Додаток2ДоПодатковоїНакладної_Функції
{
    ...
    {
        Додаток2ДоПодатковоїНакладної_Objest Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            Додаток2ДоПодатковоїНакладної_Objest Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        Додаток2ДоПодатковоїНакладної_Objest? Обєкт = await new Додаток2ДоПодатковоїНакладної_Pointer(uniqueID).GetDocumentObject(true);
