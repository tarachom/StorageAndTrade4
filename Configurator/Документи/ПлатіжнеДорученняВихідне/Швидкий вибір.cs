...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ПлатіжнеДорученняВихідне_Записи;
using Функції = StorageAndTrade.ПлатіжнеДорученняВихідне_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_4W6eASCUlnuuKND8U9ShGw")]
public partial class ПлатіжнеДорученняВихідне_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = ПлатіжнеДорученняВихідне_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ПлатіжнеДорученняВихідне_ШвидкийВибір New()
    {
        ПлатіжнеДорученняВихідне_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ПлатіжнеДорученняВихідне_Pointer(uid));
    }
