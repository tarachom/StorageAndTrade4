...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.Комплектація_Записи;
using Функції = StorageAndTrade.Комплектація_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_l26eAeWzKHuJxsFuSPrrYQ")]
public partial class Комплектація_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = Комплектація_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static Комплектація_ШвидкийВибір New()
    {
        Комплектація_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new Комплектація_Pointer(uid));
    }
