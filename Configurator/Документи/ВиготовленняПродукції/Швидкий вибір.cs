...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.ВиготовленняПродукції_Записи;
...
public partial class ВиготовленняПродукції_ШвидкийВибір : DocumentFormJournalSmall
{
    ...
    {
        TypeName = ВиготовленняПродукції_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static ВиготовленняПродукції_ШвидкийВибір New()
    {
        ВиготовленняПродукції_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new ВиготовленняПродукції_Pointer(uid));
    }
