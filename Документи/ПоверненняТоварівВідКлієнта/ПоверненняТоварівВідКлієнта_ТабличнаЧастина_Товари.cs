...
[GObject.Subclass<DocumentFormTablePart>("TablePart_mp5fsA50iEGhBG1pjDMDwA")]
partial class ПоверненняТоварівВідКлієнта_ТабличнаЧастина_Товари : DocumentFormTablePart
{
    #region Data

    [GObject.Subclass<GObject.Object>("ItemRow_mp5fsA50iEGhBG1pjDMDwA")]
    public partial class ItemRow : IRowSubclassTablePart
    {
        ...
        /* ДокументРеалізації */
        public РеалізаціяТоварівТаПослуг_Pointer ДокументРеалізації
        {
            get => ДокументРеалізації_;
            set
            {
                if (!ДокументРеалізації_.Equals(value))
                {
                    ДокументРеалізації_ = value;
                    Сhanged_ДокументРеалізації?.Invoke();
                }
            }
        }
        РеалізаціяТоварівТаПослуг_Pointer ДокументРеалізації_ = new();
        public Action? Сhanged_ДокументРеалізації { get; set; } = null;
        ...
        */
        ...
        public GObject.Object Copy()
        {
            ...
            row.Ціна = Ціна;
            row.Сума = Сума;
            row.ДокументРеалізації = ДокументРеалізації.Copy();
    ...
    public ПоверненняТоварівВідКлієнта_Objest? ЕлементВласник { get; set; }
    ...
    public static ПоверненняТоварівВідКлієнта_ТабличнаЧастина_Товари New()
    {
        ПоверненняТоварівВідКлієнта_ТабличнаЧастина_Товари tablePart = NewWithProperties([]);
    ...
    protected override void Columns()
    {
        ...
        //ДокументРеалізації
        {
            ...
            {
                if (args.Object is not ListItem listItem) return;
                var cell = РеалізаціяТоварівТаПослуг_PointerTablePartCell.New();
            ...
            {
                if (args.Object is not ListItem listItem) return;
                if (listItem.Child is not РеалізаціяТоварівТаПослуг_PointerTablePartCell cell) return;
                if (listItem.Item is not ItemRow row) return;

                cell.OnSelect = () => row.ДокументРеалізації = cell.Pointer;
                (row.Сhanged_ДокументРеалізації = () => cell.Pointer = row.ДокументРеалізації).Invoke();
    ...
    public override async Task LoadRecords()
    {
        ...
        {
            ЕлементВласник.Товари_TablePart.FillJoin([ПоверненняТоварівВідКлієнта_Товари_TablePart.НомерРядка,]);
            ...
            foreach (var record in ЕлементВласник.Товари_TablePart.Records)
            {
                ...
                row.ДокументРеалізації = record.ДокументРеалізації;
        ...
        if (ЕлементВласник != null)
        {
            ...
            for (uint i = 0; i <= Store.GetNItems(); i++)
            {
                ...
                if (row != null)
                {
                    ЕлементВласник.Товари_TablePart.Records.Add(new()
                    {
                        ...
                        Ціна = row.Ціна,
                        Сума = row.Сума,
                        ДокументРеалізації = row.ДокументРеалізації,
                    });
                }
            }
            ...
            //Оновлення табличної частини після збереження
            {
                ...
                var rows = ЕлементВласник.Товари_TablePart.Records.Select(x =>
                {
                    ...
                    row.Ціна = x.Ціна;
                    row.Сума = x.Сума;
                    row.ДокументРеалізації = x.ДокументРеалізації;
