...
static class КорегуванняБоргу_Функції
{
    ...
    {
        КорегуванняБоргу_Елемент page = КорегуванняБоргу_Елемент.New();
    ...
    public static async Task OpenPageList(UniqueID? uniqueID = null, Action<UniqueID>? сallBack_OnSelectPointer = null)
    {
        ...
        Program.BasicForm?.NotebookFunc.CreatePage(КорегуванняБоргу_Const.FULLNAME, page);
        await page.SetValue();
    }

    public static async Task SetDeletionLabel(UniqueID uniqueID)
    {
        КорегуванняБоргу_Pointer Вказівник = new(uniqueID);
    ...
    {
        КорегуванняБоргу_Object Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            КорегуванняБоргу_Object Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        КорегуванняБоргу_Object? Обєкт = await new КорегуванняБоргу_Pointer(uniqueID).GetDocumentObject(true);
