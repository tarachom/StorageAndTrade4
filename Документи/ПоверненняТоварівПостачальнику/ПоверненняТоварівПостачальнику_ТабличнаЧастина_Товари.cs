...
[GObject.Subclass<DocumentFormTablePart>("TablePart_extnW2ywjkiKj8LLUZksBA")]
partial class ПоверненняТоварівПостачальнику_ТабличнаЧастина_Товари : DocumentFormTablePart
{
    #region Data

    [GObject.Subclass<GObject.Object>("ItemRow_extnW2ywjkiKj8LLUZksBA")]
    public partial class ItemRow : IRowSubclassTablePart
    {
        ...
        /* ДокументПоступлення */
        public ПоступленняТоварівТаПослуг_Pointer ДокументПоступлення
        {
            get => ДокументПоступлення_;
            set
            {
                if (!ДокументПоступлення_.Equals(value))
                {
                    ДокументПоступлення_ = value;
                    Сhanged_ДокументПоступлення?.Invoke();
                }
            }
        }
        ПоступленняТоварівТаПослуг_Pointer ДокументПоступлення_ = new();
        public Action? Сhanged_ДокументПоступлення { get; set; } = null;
        ...
        */
        ...
        public GObject.Object Copy()
        {
            ...
            row.Ціна = Ціна;
            row.Сума = Сума;
            row.ДокументПоступлення = ДокументПоступлення.Copy();
    ...
    public static ПоверненняТоварівПостачальнику_ТабличнаЧастина_Товари New()
    {
        ПоверненняТоварівПостачальнику_ТабличнаЧастина_Товари tablePart = NewWithProperties([]);
    ...
    protected override void Columns()
    {
        ...
        //ДокументПоступлення
        {
            ...
            {
                if (args.Object is not ListItem listItem) return;
                var cell = ПоступленняТоварівТаПослуг_PointerTablePartCell.New();
            ...
            {
                if (args.Object is not ListItem listItem) return;
                if (listItem.Child is not ПоступленняТоварівТаПослуг_PointerTablePartCell cell) return;
                if (listItem.Item is not ItemRow row) return;

                cell.OnSelect = () => row.ДокументПоступлення = cell.Pointer;
                (row.Сhanged_ДокументПоступлення = () => cell.Pointer = row.ДокументПоступлення).Invoke();
    ...
    public override async Task LoadRecords()
    {
        if (ЕлементВласник != null)
        {
            ...
            foreach (var record in ЕлементВласник.Товари_TablePart.Records)
            {
                ...
                row.ДокументПоступлення = record.ДокументПоступлення;
        ...
        if (ЕлементВласник != null)
        {
            ...
            for (uint i = 0; i <= Store.GetNItems(); i++)
            {
                ...
                if (row != null)
                {
                    ЕлементВласник.Товари_TablePart.Records.Add(new()
                    {
                        ...
                        Ціна = row.Ціна,
                        Сума = row.Сума,
                        ДокументПоступлення = row.ДокументПоступлення,
            ...
            //Оновлення табличної частини після збереження
            {
                ...
                var rows = ЕлементВласник.Товари_TablePart.Records.Select(x =>
                {
                    ...
                    row.Ціна = x.Ціна;
                    row.Сума = x.Сума;
                    row.ДокументПоступлення = x.ДокументПоступлення;
