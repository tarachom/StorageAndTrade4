...
using ТабличнийСписок = GeneratedCode.Документи.ТабличніСписки.БухгалтерськаОперація_Записи;
using Функції = StorageAndTrade.БухгалтерськаОперація_Функції;

namespace StorageAndTrade;

[GObject.Subclass<DocumentFormJournalSmall>("SmallList_0ohbJfIXvkW1HXLUy5UHfw")]
public partial class БухгалтерськаОперація_ШвидкийВибір : DocumentFormJournalSmall
{
    partial void Initialize()
    {
        TypeName = БухгалтерськаОперація_Const.POINTER;
        KeyForSetting = ".Small";
        ТабличнийСписок.AddColumn(this);
        SetPagesSettings(50);
    }

    public static БухгалтерськаОперація_ШвидкийВибір New()
    {
        БухгалтерськаОперація_ШвидкийВибір list = NewWithProperties([]);
    ...
    {
        foreach (var uid in uniqueID)
            CommonForms_DocumentMovementThroughRegisters.Create(new БухгалтерськаОперація_Pointer(uid));
    }
