...
{
    public static async Task<bool> Spend(РахунокФактура_Object ДокументОбєкт)
    {
        try
        {
            #region Підготовка
            ...
            foreach (РахунокФактура_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
            {
                ...
                if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
                {
                    ...
                    if (recordResult.Result)
                    {
                        ...
                        if (ВНаявності - ВРезервіЗіСкладу < ТовариРядок.Кількість * ТовариРядок.Коєфіціент)
                        {
                            throw new Exception($"Рядок {ТовариРядок.НомерРядка}. " +
                                ...
                                $"потрібно зарезервувати {ТовариРядок.Кількість * ТовариРядок.Коєфіціент}");
                        }
                    }
                    else
                    {
            ...
            foreach (РахунокФактура_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
            {
                if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
                {
                    ВільніЗалишки_RecordsSet.Record record = new()
                    {
                        ...
                        ВРезервіЗіСкладу = ТовариРядок.Кількість * ТовариРядок.Коєфіціент,
    ...
    public static async Task Clear(РахунокФактура_Object ДокументОбєкт)
    {
        #region Бух
