...
static class Амортизація_Функції
{
    public static List<Where> Відбори(string searchText)
    {
        ...
        [
            ...
            //Назва
            new Where(Амортизація_Const.Назва, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
                    
            //Коментар
            new Where(Comparison.OR, Амортизація_Const.Коментар, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
    ...
    {
        Амортизація_Елемент page = Амортизація_Елемент.New();
    ...
    {
        Амортизація_Список page = Амортизація_Список.New();
        page.DocumentPointerItem = uniqueID;
        page.CallBack_OnSelectPointer = сallBack_OnSelectPointer;

        Program.BasicForm?.NotebookFunc.CreatePage(Амортизація_Const.FULLNAME, page);
        await page.SetValue();
    }

    public static async Task SetDeletionLabel(UniqueID uniqueID)
    {
        Амортизація_Pointer Вказівник = new(uniqueID);
