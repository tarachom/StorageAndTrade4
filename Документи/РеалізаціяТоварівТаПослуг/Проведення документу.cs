        ...
        try
        {
            ...
            foreach (РеалізаціяТоварівТаПослуг_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
            {
                ...
                if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
                {
                    ...
                    {
                        if (ЗалишокНоменклатури[ТовариРядок.НомерРядка] < ТовариРядок.Кількість * ТовариРядок.Коєфіціент)
                        {
                            throw new Exception($"Рядок {ТовариРядок.НомерРядка}. " +
                                $"Залишок товару {ЗалишокНоменклатури[ТовариРядок.НомерРядка]}, потрібно {ТовариРядок.Кількість * ТовариРядок.Коєфіціент}");
                        }
                        ...
                        if (РезервНоменклатури[ТовариРядок.НомерРядка] > 0)
                        {
                            ...
                            if (ЗалишокНоменклатури[ТовариРядок.НомерРядка] - РезервЗагальний < ТовариРядок.Кількість * ТовариРядок.Коєфіціент)
                            {
                                if (РезервДокументівСума == 0)
                                    throw new Exception($"Рядок {ТовариРядок.НомерРядка}. " +
                                        $"Залишок товару {ЗалишокНоменклатури[ТовариРядок.НомерРядка]}, " +
                                        $"в резерві {РезервЗагальний}, " +
                                        $"потрібно {ТовариРядок.Кількість * ТовариРядок.Коєфіціент}");
                                else
                                    throw new Exception($"Рядок {ТовариРядок.НомерРядка}. " +
                                        $"Залишок товару {ЗалишокНоменклатури[ТовариРядок.НомерРядка]}, " +
                ...
                if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
                {
                    if (!ТовариРядок.ЗамовленняКлієнта.IsEmpty())
                    {
                        ...
                        ЗамовленняКлієнтів_RecordsSet.Record record = new()
                        {
                            ...
                            Замовлено = Замовлено > ТовариРядок.Кількість * ТовариРядок.Коєфіціент ? ТовариРядок.Кількість * ТовариРядок.Коєфіціент : Замовлено,
                            Сума = Замовлено > ТовариРядок.Кількість * ТовариРядок.Коєфіціент ? 0 : ЗамовленняСума
                        };
            ...
            foreach (РеалізаціяТоварівТаПослуг_Товари_TablePart.Record ТовариРядок in ДокументОбєкт.Товари_TablePart.Records)
            {
                if (СписокНоменклатури[ТовариРядок.НомерРядка].ТипНоменклатури == Перелічення.ТипиНоменклатури.Товар)
                {
                    decimal КількістьЯкуПотрібноРозприділити = ТовариРядок.Кількість * ТовариРядок.Коєфіціент;
                    ...
                    foreach (Dictionary<string, object> nameRow in recordResult.ListRow)
                    {
                        ...
                        //Продажі
                        {
                            Продажі_RecordsSet.Record record = new()
                            {
                                ...
                                ХарактеристикаНоменклатури = ТовариРядок.ХарактеристикаНоменклатури,
                                Кількість = КількістьЩоСписується,
                                Сума = (ТовариРядок.Ціна / ТовариРядок.Коєфіціент) * КількістьЩоСписується,
