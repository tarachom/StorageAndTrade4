...
static class РеалізаціяТоварівТаПослуг_Функції
{
    ...
    {
        РеалізаціяТоварівТаПослуг_Елемент page = РеалізаціяТоварівТаПослуг_Елемент.New();
    ...
    public static async Task OpenPageList(UniqueID? uniqueID = null, Action<UniqueID>? сallBack_OnSelectPointer = null)
    {
        ...
        Program.BasicForm?.NotebookFunc.CreatePage(РеалізаціяТоварівТаПослуг_Const.FULLNAME, page);
        await page.SetValue();
    }

    public static async Task SetDeletionLabel(UniqueID uniqueID)
    {
        РеалізаціяТоварівТаПослуг_Pointer Вказівник = new(uniqueID);
    ...
    {
        РеалізаціяТоварівТаПослуг_Object Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            РеалізаціяТоварівТаПослуг_Object Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        РеалізаціяТоварівТаПослуг_Object? Обєкт = await new РеалізаціяТоварівТаПослуг_Pointer(uniqueID).GetDocumentObject(true);
