...
[GObject.Subclass<DocumentFormTablePart>("TablePart_GAMUmOT1v0W4a2t5FLnhA")]
partial class РеалізаціяТоварівТаПослуг_ТабличнаЧастина_Товари : DocumentFormTablePart
{
    #region Data

    [GObject.Subclass<GObject.Object>("ItemRow_GAMUmOT1v0W4a2t5FLnhA")]
    public partial class ItemRow : IRowSubclassTablePart
    {
        ...
        public Action? Сhanged_ЗамовленняКлієнта { get; set; } = null;
        ...
        public Action? Сhanged_РахунокФактура { get; set; } = null;
        ...
        */
        ...
        public GObject.Object Copy()
        {
            ...
            row.Сума = Сума;
            row.Скидка = Скидка;
            row.ЗамовленняКлієнта = ЗамовленняКлієнта.Copy();
            row.РахунокФактура = РахунокФактура.Copy();
    ...
    {
        ВидиЦін_Pointer ВидЦіни = await ЗначенняТипові.ОсновнийВидЦіни();
    ...
    async Task ОтриматиЦіну(ItemRow row)
    {
        if (row.Сума != 0)
            return;
...
";
    ...
    public static РеалізаціяТоварівТаПослуг_ТабличнаЧастина_Товари New()
    {
        РеалізаціяТоварівТаПослуг_ТабличнаЧастина_Товари tablePart = NewWithProperties([]);
    ...
    protected override void Columns()
    {
        ...
        //РахунокФактура
        {
            ...
            {
                if (args.Object is not ListItem listItem) return;
                var cell = РахунокФактура_PointerTablePartCell.New();
            ...
            {
                if (args.Object is not ListItem listItem) return;
                if (listItem.Child is not РахунокФактура_PointerTablePartCell cell) return;
                if (listItem.Item is not ItemRow row) return;
    ...
    public override async Task LoadRecords()
    {
        if (ЕлементВласник != null)
        {
            ...
            foreach (var record in ЕлементВласник.Товари_TablePart.Records)
            {
                ...
                row.ЗамовленняКлієнта = record.ЗамовленняКлієнта;
                row.РахунокФактура = record.РахунокФактура;
        ...
        if (ЕлементВласник != null)
        {
            ...
            //Оновлення табличної частини після збереження
            {
                ...
                var rows = ЕлементВласник.Товари_TablePart.Records.Select(x =>
                {
                    ...
                    row.Сума = x.Сума;
                    row.Скидка = x.Скидка;
                    row.ЗамовленняКлієнта = x.ЗамовленняКлієнта;
                    row.РахунокФактура = x.РахунокФактура;
                    row.Склад = x.Склад;
                    row.Партія = x.Партія;
