...
static class ПерерахунокТоварів_Функції
{
    public static List<Where> Відбори(string searchText)
    {
        ...
        [
            ...
            //Коментар
            new Where(Comparison.OR, ПерерахунокТоварів_Const.Коментар, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
                    
            //КлючовіСловаДляПошуку
            new Where(Comparison.OR, ПерерахунокТоварів_Const.КлючовіСловаДляПошуку, Comparison.LIKE, searchText) { FuncToField = "LOWER" },
    ...
    {
        ПерерахунокТоварів_Pointer Вказівник = new(uniqueID);
    ...
    {
        ПерерахунокТоварів_Object Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            ПерерахунокТоварів_Object Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        ПерерахунокТоварів_Object? Обєкт = await new ПерерахунокТоварів_Pointer(uniqueID).GetDocumentObject(true);
