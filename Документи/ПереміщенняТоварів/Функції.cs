...
static class ПереміщенняТоварів_Функції
{
    ...
    {
        ПереміщенняТоварів_Елемент page = ПереміщенняТоварів_Елемент.New();
    ...
    public static async Task OpenPageList(UniqueID? uniqueID = null, Action<UniqueID>? сallBack_OnSelectPointer = null)
    {
        ...
        Program.BasicForm?.NotebookFunc.CreatePage(ПереміщенняТоварів_Const.FULLNAME, page);
        await page.SetValue();
    }
    ...
    {
        ПереміщенняТоварів_Object Обєкт = new();
        if (await Обєкт.Read(uniqueID))
        {
            ПереміщенняТоварів_Object Новий = await Обєкт.Copy(true);
            await Новий.Save();
    ...
    {
        ПереміщенняТоварів_Object? Обєкт = await new ПереміщенняТоварів_Pointer(uniqueID).GetDocumentObject(true);
